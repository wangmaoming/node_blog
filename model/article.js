...
module.exports = class Article extends require('./model') {
     ...
     */
    static getListByCategoryId(id,start, size) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT id,title,content,`time`,`thumbnail` FROM article WHERE category_id = ? ORDER BY TIME DESC LIMIT ?,?'
    ...
    static getPrevArticle(id) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT id,title,`thumbnail` FROM article WHERE id < ? ORDER BY id DESC LIMIT 1'
            this.query(sql, id).then(results => {
                resolve(results[0])
            }).catch(err => {
    ...
    static getNextArticle(id) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT id,title,`thumbnail` FROM article WHERE id > ? ORDER BY id ASC LIMIT 1'
            this.query(sql, id).then(results => {
                resolve(results[0])
            }).catch(err => {
